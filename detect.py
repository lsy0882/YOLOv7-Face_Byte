...
from byte_tracker.BYTETracker import BYTETracker
import yaml
import pdb
...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                # Process tracking
                trk_results = tracker.update(det[:,:5].cpu().numpy(), im0.shape[0]*im0.shape[1])

                # Write results
                for trk_result in trk_results:
                    xyxy = trk_result.tlbr
                    conf = trk_result.score
                    cls = trk_result.cls
                    track_id = str(trk_result.track_id)
                    
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        c = int(cls)  # integer class
                        label = None if opt.hide_labels else (names[c] if opt.hide_conf else f'{names[c]} {conf:.2f}')
                        plot_one_box(track_id, xyxy, im0, label=label, color=colors(c, True), line_thickness=opt.line_thickness, kpt_label=None, kpts=None, steps=3, orig_shape=im0.shape[:2])
            ...
            if each_fps >= max_fps:
                max_fps = each_fps
            print(f'\r{s}Each frame infer.  {t2 - t1:.3f} s.  {each_fps:.3f} fps)', end="")
    ...
    print(f'Infer + Save Total Done.  {time.time() - t0:.3f} s')
    print(f'Infer Average FPS: {total_fps / frame_idx:.3f} fps')
    print(f'Infer Minimum FPS: {min_fps:.3f} fps')
    print(f'Infer Maximum FPS: {max_fps:.3f} fps')
